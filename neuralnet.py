...
class NeuralNetMLP(object):
  ...
  def _forwardProp(self, X):
    #1. net input of hidden layer
    z_h = np.dot(X, self.w_h) + self.b_h

    #2. activate hidden layer
    a_h = self._sigmoid(z_h)

    #3. net input of output layer
    z_out = np.dot(a_h, self.w_out) + self.b_out

    #4. activate output layer
  ...
  def predict(self, X):
    z_h, a_h, z_out, a_out = self._forwardProp(X)
    y_pred = np.argmax(z_out, axis=1)

    return y_pred
  
  def fit(self, X_train, y_train, X_valid, y_valid):
    n_output = np.unique(y_train).shape[0] #num class labels
    ...
    for i in range(self.epochs):
      ...
      for start_idx in range(0, indices.shape[0] - self.minibatch_size + 1, self.minibatch_size):
        ...
        z_h, a_h, z_out, a_out = self._forwardProp(X_train[batch_idx])
        ...
        grad_w_out = np.sum(a_h.T, sigma_out)
